...
public class MeterNumberPicker extends View {
      ...
      if(attributesArray == null) {
         textSize = spToPx(textSize);
          minWidth = (int) dpToPx(minWidth);
            ...
            paddingHorizontal = attributesArray.getDimensionPixelSize(R.styleable.MeterNumberPicker_mnp_paddingHorizontal, (int) dpToPx(paddingHorizontal));
            paddingVertical = attributesArray.getDimensionPixelSize(R.styleable.MeterNumberPicker_mnp_paddingVertical, (int) dpToPx(paddingVertical));
        }
    ...
    private int calculateAdjustedValueOffset(int measuredHeight) {
        return Math.abs(currentScrollOffset) < measuredHeight / 2 ? currentValueOffset : currentValueOffset + (currentScrollOffset < 0 ? -1 : 1);
    }
